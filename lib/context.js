...
const debug = require('debug')('8888888888:koa-session:context');
...
class ContextSession {
  ...
  get() {
    ...
    debug('now this.session is created',this.session)
    return this.session;
  }
  ...
  set(val) {
    debug('[_CONTEXT_SESSION] set:',val)
    if (val === null) {
      this.session = false;
      return;
    }
  ...
  async save(changed) {
    ...
    debug("this.externalKey",this.externalKey)
